...
def mask_source(text: str) -> str:
    """Mask semicolon comments and quoted strings, preserving positions/newlines."""
    out = list(text)
    i = 0
    n = len(text)
    in_string = False
...
def main() -> None:
    ...
    def add_edge(direction, source_id, target_id, kind, delta, source_line, source_text, evidence):
        ...
        rows.append({
            ...
            "cross_boundary": "YES" if source_region != target_region else "NO",
    ...
    # Every R07 rule contributes its source-order fall-through edge and any explicit jump edge.
    for rid in range(R07_START, R07_END + 1):
        _, line_start, _, body = by_id[rid]
        add_edge(
            "OUTGOING", rid, rid + 1, "fall-through", None, line_start,
            f"rule {rid} -> rule {rid + 1}", "MECHANICALLY_DERIVED_SOURCE_ORDER"
        )
        for m in re.finditer(r"\(up-jump-rule\s+([^\)]+)\)", body):
            raw_delta = m.group(1).strip()
            delta = int(raw_delta)
            target = rid + delta
            if not (1 <= target <= len(rules)):
                raise SystemExit(f"OUT-OF-RANGE JUMP: rule {rid} delta {delta} -> {target}")
            add_edge("OUTGOING", rid, target, "up-jump", delta, line_start, m.group(0), "DIRECT_DONOR_TEXT")

    # Every explicit donor jump whose target enters R07 is an incoming edge.
    for rid, line_start, _, body in rules:
        for m in re.finditer(r"\(up-jump-rule\s+([^\)]+)\)", body):
            ...
            if target in r07_ids and rid not in r07_ids:
                add_edge("INCOMING", rid, target, "up-jump", delta, line_start, m.group(0), "DIRECT_DONOR_TEXT")

    # Source-order entry/exit edges are included explicitly as cross-boundary edges.
    add_edge("INCOMING", R07_START - 1, R07_START, "fall-through", None,
             by_id[R07_START - 1][1], f"rule {R07_START - 1} -> rule {R07_START}",
             "MECHANICALLY_DERIVED_SOURCE_ORDER")
    add_edge("OUTGOING", R07_END, R07_END + 1, "fall-through", None,
    ...
    for row in rows:
        ...
        unique[key] = row
    rows = sorted(unique.values(), key=lambda r: (r["source_rule"], r["direction"], r["target_rule"], r["edge_kind"]))
