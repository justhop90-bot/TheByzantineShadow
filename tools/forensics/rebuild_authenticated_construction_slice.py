...
def main() -> None:
    ...
    # Preserve the module header and compatibility declarations before the first
    # runtime rule. Replace every runtime rule with the exact authenticated donor
    # interval 1794-1949. This removes the previously observed nine-rule ID drift
    # where donor 1806-1814 bodies had been relabeled as 1815-1823.
    ...
    generated = [
        ...
        "",
        "; ============================================================",
        "; AUTHENTICATED DONOR TRANSPLANT 1794-1949",
        "; Canonical donor blob SHA: 70a18a3b69e8ea46bd5132673fe9fcf8a36595ee",
        "; Rule bodies below are copied from the authenticated donor interval in",
        "; exact source order. Comments identify evidence only; they do not alter",
        "; donor semantics. Runtime qualification remains pending.",
        "; ============================================================",
        "",
    ]
    ...
    # Re-read the generated implementation and prove exact rule-body identity.
    ...
    with INVENTORY.open("w", newline="", encoding="utf-8") as fh:
        ...
        writer.writerow([
            "rule_id", "region", "donor_blob_sha", "donor_rule_ordinal",
            "rule_body_sha256", "jump_deltas", "source_order", "evidence_class",
            "runtime_qualification"
        ])
        for rid, rule in zip(range(FIRST, LAST + 1), donor_slice):
            writer.writerow([
                rid, region(rid), donor_sha, rid, rule_sha(rule),
                ";".join(map(str, jump_deltas(rule))) or "NONE",
                "DIRECT_DONOR_SOURCE_ORDER", "DIRECT_DONOR_TEXT", "PENDING"
            ])
